...
sudo mkdir -p "$BIN_DEST" "$CONFIG_DEFAULT" "$ASSETS_DEST" "$SYSTEMD_GLOBAL"
...
# 3b. Instalar assets padrão (ex.: imagem de ausência/privacidade) em /opt
if [ -d "$BASE_DIR/assets" ]; then
    echo -e "${YELLOW}--> Instalando assets padrão em $ASSETS_DEST...${NC}"
    sudo cp -a "$BASE_DIR/assets/"* "$ASSETS_DEST/" 2>/dev/null || true
fi
