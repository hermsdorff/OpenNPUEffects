...
class CameraCtrlsWindow(Gtk.ApplicationWindow):
    ...
    def init_gui_device(self):
        ...
        self.frame = Gtk.Grid(hexpand=True, vexpand=True, halign=Gtk.Align.FILL, valign=Gtk.Align.FILL)
        self.grid.attach(self.frame, 0, 0, 1, 1)

        stack_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, margin=10, vexpand=True, hexpand=True, halign=Gtk.Align.FILL)
        ...
        scrolledwindow = Gtk.ScrolledWindow(child=stack, propagate_natural_height=False, hscrollbar_policy=Gtk.PolicyType.NEVER, vscrollbar_policy=Gtk.PolicyType.ALWAYS)
        scrolledwindow.set_min_content_height(500)
        scrolledwindow.set_min_content_width(460)
        stack_box.pack_start(scrolledwindow, True, True, 0)
        ...
        target_w = max(500, natsize.width, current_w)
        target_h = max(680, natsize.height, current_h)
